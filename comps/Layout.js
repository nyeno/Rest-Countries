...
const Layout = ({ children }) => {
  ...
  const renderThemeChanger = () => {
    ...
    if(currentTheme === 'dark') {
      return (
        <button className="space-x-2 lg:space-x-4 font-semibold"
  ...
  return (
    ...
    <Head>
        ...
        <meta name="description" content="Countries of the planet with theme switcher" />
      ...
      <div>
        <header className="flex justify-between z-50 lg:px-12 px-6 py-6 bg-white dark:bg-dark-element drop-shadow sticky top-0"> 
            <h1 className='font-extrabold lg:text-2xl text-md'>Where in the world?</h1>
            {renderThemeChanger()}
        </header>
        {children}
        <footer className='mt-8 text-center'>Created by Eno Nyenooke Laura</footer>
    </div>
    </>
